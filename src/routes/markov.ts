...
function getMarkovMatrix(sequence:string, asProbs:boolean = true) {
    ...
    let MM = Array.from({ length: dimsize }, () => Array(dimsize).fill(0));
...
export function markov_authenticate(right_sequence:string,input_sequence:string){
    let MM = getMarkovMatrix(right_sequence,true);
    console.log("\n\n\nYou entered:  " + input_sequence);

    let p_input_seq = getSeqProb(MM,input_sequence)
    let p_right_seq = getSeqProb(MM,right_sequence)
    ...
    let auth_criteria_1 = p_input_seq / p_right_seq > 0.05;
    let auth_criteria_2 = p_input_seq / p_right_seq < 1.95;
    let auth_criteria = auth_criteria_1 && auth_criteria_2;

<script lang='ts'>
    ...
    function handleDU(v:string){
        ...
        setTimeout(()=>{show_input_confirmation = false},200)
    ...
    function handleAction(b:string,pass:string){
        if(b === "clear"){
            ...
            setTimeout(()=>{show_clear_confirmation = false},1200)
        } else {
            dispatch('clr_ent',{
                button:b,
                password:pass
...
<section>
    ...
    <div id = "submit">
        ...
        <button class="clr_ent" on:click={() => handleAction('enter',user_entered_password)}>Enter</button>
    </div>
